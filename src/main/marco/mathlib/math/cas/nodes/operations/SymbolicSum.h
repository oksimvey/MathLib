...
class SymbolicSum : public SymbolicNode {
  ...
  public:
    ...
    [[nodiscard]] NodePtr simplify() const override { return CASSimplifier::simplifySum(elements); }
    ...
    [[nodiscard]] std::string toString() const override {
        ...
        for (size_t i = 0; i < elements.size(); ++i) {
            ...
            bool negative = dynamic_cast<const SymbolicNegativeNode*>(node.get()) != nullptr;
            ...
            if (negative) {
                auto* neg = static_cast<const SymbolicNegativeNode*>(node.get());
